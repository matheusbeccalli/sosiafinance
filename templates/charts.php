...
<?php
  ...
  foreach($positions_by_issuer as $position):
    $pie_data_issuer[] = array($position['issuer_name'], $position['issuer_total']);
  endforeach;
?>
  ...
  google.charts.setOnLoadCallback(drawChartByIssuer);
  ...
  function drawChartByIssuer() {

    // Create the data table.
    var data = new google.visualization.DataTable();
    data.addColumn('string', 'Issuer');
    data.addColumn('number', 'Value');
    data.addRows(<?php echo json_encode($pie_data_issuer, JSON_NUMERIC_CHECK); ?>);

    // Set chart options
    var options = {'title':'By issuer',
            'width':400,
            'height':300};

    // Instantiate and draw our chart, passing in some options.
    var chart = new google.visualization.PieChart(document.getElementById('chart_div_by_issuer'));
    chart.draw(data, options);
  }
...
<tr>
  <td><div id="chart_div_by_issuer"></div></td>
  <td></td>
</tr>
</table>
